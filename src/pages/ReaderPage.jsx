...
function ReaderPage() {
  ...
  // Tự động trích xuất ID và tạo link Tải xuống trực tiếp từ Google Drive
  ...
  if (!book) {
    return (
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#fdfbf2] text-red-800 font-['Lora',serif] font-bold text-xl animate-pulse">
        Đang tải tài liệu...
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen relative flex flex-col items-center overflow-hidden">
      ...
      <div className="w-full z-50 p-4 md:p-6 flex justify-between items-center bg-gradient-to-b from-white/90 to-transparent gap-2">
        ...
        <button
          onClick={() => navigate(-1)}
          className="bg-red-700 hover:bg-red-800 transition-colors text-white font-['Lora',serif] font-bold py-2 px-3 md:px-6 rounded shadow-md active:scale-95 flex items-center gap-1 md:gap-2 shrink-0"
        >
        ...
        {/* TÊN SÁCH Ở GIỮA */}
        {/* ĐÃ SỬA: Thay đổi font-serif mặc định thành font Lora */}
        <div className="text-red-900 font-['Lora',serif] italic text-sm md:text-xl font-bold bg-white/80 px-4 md:px-6 py-1.5 md:py-2 rounded-full shadow-sm max-w-[40%] md:max-w-[60%] truncate text-center">
          {book.title}
        </div>
        ...
        <a
          ...
          className="bg-gray-800 hover:bg-gray-900 transition-colors text-white font-['Lora',serif] font-bold py-2 px-3 md:px-6 rounded shadow-md active:scale-95 flex items-center gap-1 md:gap-2 shrink-0"
        >
      ...
      <div className="relative z-10 w-full max-w-6xl h-[80vh] md:h-[88vh] px-2 md:px-6 pb-6 mt-2">
        <div className="w-full h-full bg-white rounded-xl shadow-2xl overflow-hidden border-2 border-red-800/20 relative">
          ...
          <iframe
            ...
            allow="autoplay"
          >
            <p className="font-['Lora',serif]">Trình duyệt của bạn không hỗ trợ iframe. Hãy nâng cấp trình duyệt!</p>
          </iframe>
